...
async function login(userCreds) {
  ...
  const user = users.find(
    (usr) =>
      usr.password === userCreds.password && usr.username === userCreds.username
  );
  return user;
}
async function signup(newUser) {
  const users = await _query();
  const isUserExist = users.every(
    (usr) =>
      usr.password === newUser.password || usr.username === newUser.username
  );
